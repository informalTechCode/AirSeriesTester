...
class RayNeoSession(
    ...
    private fun initializeProtocol(): RayNeoDeviceInfo? {
        ...
        if (!info.sideBySideEnabled) {
            if (!sendCommand(RayNeoProtocol.CMD_SWITCH_TO_3D)) {
                return null
            }
            if (!waitForAck(RayNeoProtocol.CMD_SWITCH_TO_3D, 1500)) {
                return null
            }
            // Matches the driver behavior that waits for panel mode to settle.
            SystemClock.sleep(5000)
            if (!sendCommand(RayNeoProtocol.CMD_ACQUIRE_DEVICE_INFO)) {
                return null
            }
            waitForDeviceInfo(1200) ?: return null
        }
    ...
    private companion object {
        const val ACTION_USB_PERMISSION = "org.freedesktop.monado.rayneotest.USB_PERMISSION"
    }
}
